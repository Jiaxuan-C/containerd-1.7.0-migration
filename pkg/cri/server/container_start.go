...
func (c *criService) startContainer(ctx context.Context, containerID, unTarClonePodIdContainerNamePath string, opts ...containerd.NewTaskOpts) (retErr error) {
	// TODO(cjx.bupt): reuse the StartContainer method
	start := time.Now()
	...
	task, err := container.NewTask(ctx, ioCreation, unTarClonePodIdContainerNamePath, taskOpts...)
	if err != nil {
