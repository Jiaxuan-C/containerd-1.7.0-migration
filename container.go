...
func (c *container) NewTask(ctx context.Context, ioCreate cio.Creator, unTarClonePodIdContainerNamePath string, opts ...NewTaskOpts) (_ Task, err error) {
	i, err := ioCreate(c.id)
	if err != nil {
		return nil, err
	}
	...
	if r.SnapshotKey != "" {
		...
		// unTarContainerRW  TODO(cjx.bupt)
		if unTarClonePodIdContainerNamePath != "" {
			tarPath := path.Join("/var/lib/kubelet/migration", unTarClonePodIdContainerNamePath, "fs.tar")
			unTarPath, _ := path.Split(strings.Split(mounts[0].Options[2], "upperdir=")[1])
			if err := UnTarContainerRW(tarPath, unTarPath); err != nil {
				return nil, err
			}
		}
		spec, err := c.Spec(ctx)
...
func UnTarContainerRW(tarball, target string) error {
